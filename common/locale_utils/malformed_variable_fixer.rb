...
module LocaleUtils
  class MalformedVariableFixer
    ...
    def fix_malformed
      ...
      #
      # Pattern 1: Inside quoted strings (single-line only)
      # "text:% {var}" or 'text:% {var}' → "text: %{var}" or 'text: %{var}'
      # Only matches single-line strings to avoid breaking multi-line YAML values
      changes += apply_fix(content, /(['"])([^'"\n]*):(%)\s+(\{[^}]+\}[^'"\n]*)\1/, '\\1\\2: \\3\\4')
    ...
    def apply_fix(content, pattern, replacement = nil, &block)
      ...
      begin
        ...
        # Rollback to original content before raising
        content.replace(original_content)

        # Build detailed error message
        error_msg = [
          "\n❌ YAML validation failed after applying pattern in file: #{file_path}",
          "   Pattern: #{pattern.inspect}",
          "   Error: #{e.message}"
        ]
        error_msg << "   Replacement: #{replacement.inspect}" unless block_given?
        error_msg << "   This transformation would have broken the YAML file."
        error_msg << "\n   Please fix the regex pattern or the source YAML content."

        raise StandardError, error_msg.join("\n")
      end
