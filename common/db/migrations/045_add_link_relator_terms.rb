...
Sequel.migration do
  ...
  up do
    ...
    enum = self[:enumeration].filter(:name => 'linked_agent_archival_record_relators').select(:id) 
   
    terms.each do |term|
      unless self[:enumeration_value].filter(:enumeration_id => enum, :value => term)
        self[:enumeration_value].insert( :enumeration_id  => enum, :value => term, :readonly => 0 )
      end
    end
