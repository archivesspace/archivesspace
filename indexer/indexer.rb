...
class ArchivesSpaceIndexer
  ...
  @@record_types = [:accession, :archival_object, :resource, :digital_object, :digital_object_component, :collection_management, :subject]
  @current_session = nil
  ...
  def self.main
    ...
    indexer.add_document_prepare_hook {|doc, record|
      if record.class.record_type == 'subject'
        doc['display_string'] = record['terms'].collect {|t| t['term']}.join(" -- ")
      end
    }
