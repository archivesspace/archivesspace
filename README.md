# Development log for EAD3 export

## 2017-08-04

* Issues fixed
* Added handling of mixed content notes that converts 2002-valid EAD into valid EAD3

## 2017-07-24
* final integration of common components from 2002 serializer
* a testable level of done-ness has been achieved

## 2017-07-21
* `<controlaccess>`, `<bibliography>`, and `<index>` complete

## 2017-07-20
* Shared serializers for notes complete

## 2017-07-19
* Shared serializer for dates (`<unitdatestructured>`,`<unidate>`) complete

## 2017-07-18
* Progress on resource-level `<did>` serialization
* Shared serializer for `<container>` and extents (`<physdesc>`,`<physdescstructured>`) complete

## 2017-07-12
* `<control>` section of EAD3 serializer complete

## 2017-07-05
* Minor progress on EAD3 serializer - added xml-model for the rng schema

## 2017-06-23
* Added 'EAD3 schema' as an option on the 'Export > Download EAD' dropdown menu.
* Updated front end JS to recognize ead3 option from dropdown and pass it to controller
* update exports_controller to pass ead3 option to backend
* updated ExportHelpers#generate_ead to add :serializer to options passed to the exporter when :ead3 option is true
* Cloned EADSerializer and changed @xmlns to the EAD3 schema URI (just to verify that it's working - does not validate)
* requests to download resource as EAD3 return an (invalid) XML file that uses the new serializer
