...
var jqtree = (function (exports) {
  ...
  class DragElement {
    constructor(_ref) {
      let { autoEscape, nodeName, offsetX, offsetY, treeElement } = _ref;
  ...
  const isNodeRecordWithChildren = (data) =>
    typeof data === "object" &&
    "children" in data &&
  ...
  class NodeElement {
    constructor(_ref) {
      let { getScrollLeft, node, tabIndex, treeElement } = _ref;
      this.getScrollLeft = getScrollLeft;
      this.tabIndex = tabIndex;
      this.treeElement = treeElement;
      this.init(node);
  ...
  class ContainerScrollParent {
    constructor(_ref) {
      let { container, refreshHitAreas } = _ref;
    ...
    getScrollParentBottom() {
      if (this.scrollParentBottom == null) {
        this.scrollParentBottom =
          this.getScrollParentTop() + this.container.clientHeight;
      }
  ...
  class DocumentScrollParent {
    constructor(_ref) {
      let { refreshHitAreas, treeElement } = _ref;
      this.refreshHitAreas = refreshHitAreas;
      this.treeElement = treeElement;
    }
    checkHorizontalScrolling(pageX) {
  ...
  const isOverflow = (overflowValue) =>
  ...
  class JqTreeWidget extends SimpleWidget {
    ...
    getNodeByHtmlElement(inputElement) {
      const element =
        inputElement instanceof HTMLElement ? inputElement : inputElement[0];
      if (!element) {
        return null;
      }
    ...
    isFocusOnTree() {
      ...
      return Boolean(
        activeElement &&
          ...
          this.containsElement(activeElement)
      );
    }
