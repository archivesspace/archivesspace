(function(exports) {
    ...
    var renderTable = function (tree) {
        ...
        tableData.getCellSync = function(i,j) {
            ...
            if (j === 0) {
                ...
                return {
                    "content": "<label class='bulk-updater-checkbox-label'><input data-rowid='"+ i +"' id='item"+i+"' value='"+filtered_tree[i]['uri']+"' type='checkbox' " + (filtered_tree[i]['selected'] ? "checked" : "") + " /></label>",
                    "rowId": i,
                }
            }

            /* Left-pad each item relative to its level */
            var spaces = '<div class="bulk-updater-spaces">';

            for (var space = 0; space < filtered_tree[i]['level']; space++) {
                spaces += '<span class="bulk-updater-space"></span>';
            }
            ...
            var content = (spaces +
                '<div class="bulk-updater-entry">' +
                '<div><label class="bulk-updater-label" for="item'+i+'">' + filtered_tree[i]['title'] + '</label></div>' +
                '<div>' + metadata + '</div>' +
                '</div>');

            if (filtered_tree[i]['children']) {
                content = '<span class="bulk-updater-has-children">' + content + '</span>';
            }
        ...
        $("#bulk_updater_table").height(idealHeight);
    ...
    exports.initBulkUpdaterTable = function (tree) {
        var flattened = flattenTree(tree);

        let bulkUpdaterFatTable = renderTable(flattened);

        $("#bulk_updater_table").on("click", ":input", function(event) {
            ...
            var offsetTop = bulkUpdaterFatTable.scroll.scrollTop;

            bulkUpdaterFatTable = renderTable(flattened);

            // navigate back to the row you just clicked
            bulkUpdaterFatTable.goTo(rowid, 0);
            bulkUpdaterFatTable.scroll.setScrollXY(0, offsetTop);
        });


        $("#cancelBulkUpdater").on("click", function(event) {
            event.preventDefault();
            $(".modal_overlay").hide();
            $(".bulk_updater_modal").hide();
        });

        $(".submit-btn").on("click", function() {
            var self = $(this);

            if (self.attr('id') == 'showBulkUpdaterModal') {
                $(".modal_overlay").show();
                $(".bulk_updater_modal").show();
                return;
            }

            if (self.attr('id') == 'downloadBulkUpdater') {
                $(".modal_overlay").hide();
                $(".bulk_updater_modal").hide();
            }
