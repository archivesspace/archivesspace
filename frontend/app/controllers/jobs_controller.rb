class JobsController < ApplicationController
  ...
  def create
    begin
      ...
      if params[:iframePOST] # IE saviour. Render the form in a textarea for the AjaxPost plugin to pick out.
        return render :partial => "jobs/form_for_iframepost", :formats => [:html], :handlers => [:erb], :status => 400
      else
        return render :partial => "jobs/form", :formats => [:html], :handlers => [:erb], :status => 400
      end
    end
  ...
  def records
    ...
    render :partial => "jobs/job_records", :formats => [:html], :handlers => [:erb]
  end
