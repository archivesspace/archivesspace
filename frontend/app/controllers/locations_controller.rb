class LocationsController < ApplicationController
  ...
  def new
    ...
    render :partial => "locations/new", :formats => [:html], :handlers => [:erb] if inline?
  end
  ...
  def create
    handle_crud(:instance => :location,
                ...
                :on_invalid => ->(){
                  return render :partial => "locations/new", :formats => [:html], :handlers => [:erb] if inline?
  ...
  def batch_create
    begin
      ...
      if batch_response.kind_of?(Hash) and batch_response.has_key?("error")
        if params["dry_run"]
          return render :partial => "shared/quick_messages", :formats => [:html], :handlers => [:erb], :locals => {:exceptions => batch_response, :jsonmodel => "location_batch"}
        else
      ...
      if params["dry_run"]
        render :partial => "locations/batch_preview", :formats => [:html], :handlers => [:erb], :locals => {:locations => batch_response}
      else
