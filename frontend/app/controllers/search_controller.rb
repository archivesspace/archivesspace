        ...
        if is_top_container_search
          ...
          self.response_body = Enumerator.new do |y|
            ...
            results.each do |result|
              begin
                # Data is directly in the result hash (Solr doc), not result['json']
                doc_for_extraction = result

                # Log the structure of the first result's hash
                unless first_result_logged
                  Rails.logger.debug("First result hash structure for CSV: #{doc_for_extraction.inspect}")
                  first_result_logged = true
                end

                # Extract data using Solr field names, handling missing values and potential arrays
