module AspaceSearchHelper
  ...
  def jsonmodel_url_for(search_result_json, action)
    case search_result_json["type"]
      when "accession"
        {:controller => :accessions, :action => action, :id => JSONModel(:accession).id_for(search_result_json["id"]),:search_token => @search_data['search_token']}
      when "resource"
        {:controller => :resources, :action => action, :id => JSONModel(:resource).id_for(search_result_json["id"]),:search_token => @search_data['search_token']}
      ...
      when "digital_object"
        {:controller => :digital_objects, :action => action, :id => JSONModel(:digital_object).id_for(search_result_json["id"]),:search_token => @search_data['search_token']}
