...
describe 'Record Lifecycle' do
  ...
  it 'can suppress an Accession' do#, skip: 'UPGRADE lost in translation'
    ...
    assert(5) { expect(@driver.find_element(css: 'div.alert.alert-success').text).to eq("Accession #{@accession.title} suppressed") }
    assert(5) { expect(@driver.find_element(css: 'div.alert.alert-info').text).to eq('Accession is suppressed and cannot be edited') }
  ...
  it "an archivist can't see a suppressed Accession" do#, skip: 'UPGRADE lost in translation'
    ...
    # No element found
    expect(@driver.find_element_with_text('//td', /#{@accession.title}/, true, true)).to be_nil
  ...
  it 'can unsuppress an Accession' do#, skip: 'UPGRADE lost in translation'
  ...
  it 'can delete an Accession' do#, skip: 'UPGRADE lost in translation'
    @driver.login_to_repo(@manager_user, @repo)
    ...
    assert(5) { expect(@driver.find_element(css: 'div.alert.alert-success').text).to eq("Accession #{@accession.title} deleted") }
  ...
  it 'can suppress a Digital Object' do#, skip: 'UPGRADE lost in translation'
    @driver.login_to_repo(@manager_user, @repo)
    ...
    assert(5) { expect(@driver.find_element(css: 'div.alert.alert-success').text).to eq("Digital Object #{@do.title} suppressed") }
    assert(5) { expect(@driver.find_element(css: 'div.alert.alert-info').text).to eq('Digital Object is suppressed and cannot be edited') }
    ...
    # @driver.wait_for_ajax
    expect(@driver.find_element(css: 'div.alert.alert-info').text).to eq('Digital Object is suppressed and cannot be edited')
  end
end
