...
describe 'Accessions', js: true do
  ...
  it 'can delete accessions from the listing' do
    # TODO: Fix this. This example requires a login with a
    # a use that belongs to `repository-managers`. Remove login
    # from before each block, and apply it individually on each example.
    visit '/logout'
    ...
    # Search for first accession and select for delete
    input_text = find('#filter-text')
    input_text.fill_in  with: accession_first.title
    input_text.send_keys(:enter)
    first_entry = find('tr', text: accession_first.title)
    first_entry_checkbox = first_entry.find('input[type="checkbox"]')
    ...
    input_text.fill_in  with: accession_second.title
    input_text.send_keys(:enter)
    second_entry = find('tr', text: accession_second.title)
    second_entry_checkbox = second_entry.find('input[type="checkbox"]')
    second_entry_checkbox.click
