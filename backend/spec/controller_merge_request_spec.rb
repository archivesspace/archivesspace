...
MERGEABLE_TYPES = ['subject', 'top_container', 'container_profile', 'agent', 'resource', 'digital_object'].freeze
...
describe 'Merge request controller' do
  ['accession', 'digital_object', 'resource', 'archival_object', 'digital_object_component'].each do |type|
    context "when merge candidate and destination are linked to parents of type: #{type}" do
      context "merging subjects" do
        let(:merge_destination) { create(:json_subject) }
        let(:merge_candidate) { create(:json_subject) }

        it "removes original relationships to merge candidate and keeps relationships to merge destination" do
      ...
      context "merging agents" do
        let(:merge_destination) { create(:json_agent_person) }
        let(:merge_candidate) { create(:json_agent_person) }

        it "deletes duplicate relationships" do
  ...
  MERGEABLE_TYPES.each do |type|
    context "when merging a #{type} with itself" do
  ...
  (MERGEABLE_TYPES - ['agent']).combination(2).to_a.each do |types|
    context "when attempting to merge a #{types[1]} into a #{types[0]}" do
