...
describe 'OAI handler' do
  ...
  def harvested_records(response)
    records = response.respond_to?(:records) ? response.records : response

    records.grep(ArchivesSpaceOAIRecord)
  end

  def identifiers_in(response)
    Nokogiri::XML(response.body)
      .xpath("//xmlns:header[not(@status='deleted')]/xmlns:identifier")
      .map(&:text)
  end

  def define_oai_sets(repository_sets = [], sponsor_sets = [])
    oai_config = OAIConfig.first
    json = OAIConfig.to_jsonmodel(oai_config.id)

    json['oai_repository_sets'] = repository_sets
    json['oai_sponsor_sets'] = sponsor_sets

    oai_config.update_from_json(json)
  end
  ...
  describe "OAI protocol and mapping support" do
    ...
    it "includes sets defined in OAIConfig table in ListSets request" do
      define_oai_sets([{'set_name' => 'repo_set_name',
                        'set_description' => 'foobar',
                        'repo_codes' => ['foo', 'bar']}],
                      [{'set_name' => 'sponsor_set_name',
                        'set_description' => 'bimbaz',
                        'sponsor_names' => ['bim', 'baz']}])
    ...
    # ANW-2707
    it "includes every repository set and sponsor set in a ListSets request" do
      define_oai_sets([{'set_name' => 'repos_1', 'set_description' => 'first', 'repo_codes' => ['foo']},
                       {'set_name' => 'repos_2', 'set_description' => 'second', 'repo_codes' => ['bar']}],
                      [{'set_name' => 'sponsors_1', 'set_description' => 'third', 'sponsor_names' => ['bim']},
                       {'set_name' => 'sponsors_2', 'set_description' => 'fourth', 'sponsor_names' => ['baz']}])

      result = ArchivesSpaceOaiProvider.new.process_request(:verb => 'ListSets')

      ['repos_1', 'repos_2', 'sponsors_1', 'sponsors_2'].each do |set_name|
        expect(result).to match(/<setSpec>#{set_name}<\/setSpec><setName>#{set_name}<\/setName>/)
      end
    end

    # migration 177 imports legacy set names, a set named after a level of description can still be in the database.
    it "never reports a configured set named after a level of description" do
      level = BackendEnumSource.values_for("archival_record_level").first

      define_oai_sets([{'set_name' => 'a_harmless_set',
                        'set_description' => 'harmless_set_description',
                        'repo_codes' => ['oai_test']}])

      OAIRepositorySet.dataset.insert(:oai_config_id       => OAIConfig.first.id,
                                      :json_schema_version => 1,
                                      :set_name            => level,
                                      :set_description     => 'shadowed_set_description',
                                      :repo_codes          => ['oai_test'].to_json,
                                      :created_by          => 'admin',
      ...
      result = ArchivesSpaceOaiProvider.new.process_request(:verb => 'ListSets')
      set_specs = result.scan(/<setSpec>([^<]+)<\/setSpec>/).flatten

      expect(set_specs).to eq(set_specs.uniq)
      expect(set_specs.count(level)).to eq(1)
      expect(result).to_not match(/shadowed_set_description/)

      expect(set_specs).to include('a_harmless_set')
      expect(result).to match(/harmless_set_description/)
    end
  ...
  describe "ListRecords" do
    ...
    it "supports OAI sets based on sponsors" do
      define_oai_sets([], [{'set_name' => 'sponsor_set',
                            'set_description' => 'sponsor_set_description',
                            'sponsor_names' => ['sponsor_0']}])
    ...
    it "supports OAI sets based on repositories" do
      define_oai_sets([{'set_name' => 'repo_set_name',
                        'set_description' => 'repo_set_description',
                        'repo_codes' => ['oai_test']}])
    ...
    # ANW-2707
    it "harvests each repository set separately" do
      define_oai_sets([{'set_name' => 'in_scope',
                        'set_description' => 'The repository holding our test records',
                        'repo_codes' => ['oai_test']},
                       {'set_name' => 'out_of_scope',
                        'set_description' => 'Some other repository',
                        'repo_codes' => ['no_such_repo_code']}])

      in_scope = harvested_records(oai_repo.find(:all, {:metadata_prefix => "oai_dc", :set => 'in_scope'}))
      expect(in_scope.length).to be > 0
      expect(in_scope.all? {|record| record.sequel_record.repo_id == @oai_repo_id}).to be(true)

      out_of_scope = harvested_records(oai_repo.find(:all, {:metadata_prefix => "oai_dc", :set => 'out_of_scope'}))
      expect(out_of_scope).to be_empty
    end

    # ANW-2707: one set can name several repositories, the point of a
    # repository set on a shared instance.
    it "harvests every repository named by a single set" do
      second_repo_id = RequestContext.open do
        FactoryBot.create(:repo,
                          :repo_code => "oai_test_second",
                          :org_code => "oai",
                          :name => "oai_test_second").id
      end

      RequestContext.open(:repo_id => second_repo_id) do
        JSONModel.with_repository(second_repo_id) do
          create(:json_resource, :publish => true)
        end
      end

      define_oai_sets([{'set_name' => 'both_repos',
                        'set_description' => 'Two repositories harvested as one set',
                        'repo_codes' => ['oai_test', 'oai_test_second']}])

      # A page big enough for both repositories' resources at once
      provider = ArchivesSpaceOAIRepository.new
      allow(provider).to receive(:options_for_type)
                           .with('oai_dc')
                           .and_return(ArchivesSpaceOAIRepository::FormatOptions.new([Resource], 25))

      records = harvested_records(provider.find(:all, {:metadata_prefix => "oai_dc", :set => 'both_repos'}))

      expect(records.map {|record| record.sequel_record.repo_id}.uniq)
        .to match_array([@oai_repo_id, second_repo_id])
    end

    # ANW-2707: likewise for several sponsors in one set
    it "harvests every sponsor named by a single set" do
      define_oai_sets([], [{'set_name' => 'two_sponsors',
                            'set_description' => 'Two sponsors harvested as one set',
                            'sponsor_names' => ['sponsor_0', 'sponsor_2']}])
      ...
      records = harvested_records(provider.find(:all, {:metadata_prefix => "oai_dc", :set => 'two_sponsors'}))

      expect(records.map {|record| record.jsonmodel_record['finding_aid_sponsor']})
        .to match_array(['sponsor_0', 'sponsor_2'])
    end

    # ANW-2707
    it "treats an empty set argument as no set at all" do
      define_oai_sets([{'set_name' => 'a_repo_set',
                        'set_description' => 'a_repo_set_description',
                        'repo_codes' => ['oai_test']}])

      unqualified = get "/oai?verb=ListIdentifiers&metadataPrefix=oai_dc"
      empty_set = get "/oai?verb=ListIdentifiers&metadataPrefix=oai_dc&set="
      blank_set = get "/oai?verb=ListIdentifiers&metadataPrefix=oai_dc&set=%20"

      expect(identifiers_in(unqualified)).to_not be_empty

      [empty_set, blank_set].each do |response|
        expect(response.body).to_not match(/<error/)
        expect(identifiers_in(response)).to eq(identifiers_in(unqualified))
      end
    end

    # ANW-2707
    it "reports noRecordsMatch for a set it doesn't know about" do
      define_oai_sets([{'set_name' => 'a_known_set',
                        'set_description' => 'A set that does exist',
                        'repo_codes' => ['oai_test']}])

      response = get "/oai?verb=ListRecords&set=not_a_set&metadataPrefix=oai_dc"

      expect(response.body).to match(/<error code="noRecordsMatch">/)

      response = get "/oai?verb=ListRecords&set=a_known_set&metadataPrefix=oai_dc"
    ...
    describe "OAI sets for resource records" do
      ...
      it "supports OAI sets based on levels for resource records too" do
        series_resource = create(:json_resource, :level => 'series', :publish => true)

        records = harvested_records(oai_repo.find(:all, {:metadata_prefix => "oai_dc", :set => 'series'}))

        expect(records.map {|record| record.jsonmodel_record['uri']}).to eq([series_resource.uri])
      end

      it "supports OAI sets based on sponsors for resource records too" do
        define_oai_sets([], [{'set_name' => 'sponsor_set_name',
                              'set_description' => 'sponsor_set_description',
                              'sponsor_names' => ['sponsor_0']}])
  ...
  describe "repository with sets disabled" do
    ...
    it "does not return an object if set excluded from OAI in repo" do
      excluded = get "/oai?verb=ListRecords&set=collection&metadataPrefix=oai_dc"
      included = get "/oai?verb=ListRecords&set=fonds&metadataPrefix=oai_dc"


      expect(excluded.body).to_not match(/<error/)
      expect(identifiers_in(excluded)).to be_empty
      expect(identifiers_in(included)).to_not be_empty
    end
