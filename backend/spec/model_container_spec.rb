...
describe 'Container model' do
  ...
  it "Should raise an error if the type_1 or barcode is not set" do
    ...
    expect { Container.create_from_json(JSONModel(:container).from_hash({"barcode_1" => "123"})) }.to_not raise_error(JSONModel::ValidationException)
    ...
    expect { Container.create_from_json(JSONModel(:container).from_hash({"type_1" => "frame", "indicator_1" => "abc"})) }.to_not raise_error(JSONModel::ValidationException)
