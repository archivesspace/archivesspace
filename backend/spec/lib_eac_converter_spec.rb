...
describe 'EAC converter' do
  ...
  describe 'people agents' do
    it 'imports primary_name' do
      record = convert(person_agent_1).select { |r| r['jsonmodel_type'] == 'agent_person' }.last
    ...
    it 'imports bioghist notes' do
      ...
      notes = record['notes'].first['subnotes']
      abstract = notes.select { |n| n['jsonmodel_type'] == 'note_abstract' }
      text = notes.select { |n| n['jsonmodel_type'] == 'note_text' }

      expect(record).not_to be_nil
      expect(notes.length).to eq(2)
      expect(abstract.first['content']).to start_with(/Richard H. Lufkin was a shoe machine engineer/)
      expect(text.first['content']).to start_with("Richard H. Lufkin (1851-1922) was an engineer")
    end
    ...
    it 'imports places as subjects' do
      ...
      expect(geo_subjects.first['source']).to eq('GND')
    ...
    it 'imports related agents' do
      full_record = convert(person_agent_1)
      agent_person_records = full_record.select { |r| r['jsonmodel_type'] == 'agent_person' }
      agent_corp_records = full_record.select { |r| r['jsonmodel_type'] == 'agent_corporate_entity' }

      expect(agent_person_records.length).to eq(13)
      expect(agent_corp_records.length).to eq(1)

      # relationships are there
      expect(agent_person_records.last['related_agents']).not_to be_nil
      expect(agent_person_records.last['related_agents'].first['jsonmodel_type']).to eq('agent_relationship_associative')
      # related agents are there
      expect(agent_person_records.first['names'][0]['primary_name']).to eq('Pauling')
      expect(agent_corp_records.first['names'][0]['primary_name']).to eq('California Institute of Technology.')
    end
  ...
  describe 'corporate agents' do
    it 'imports part with no attrs as primary_name' do
      record = convert(corp_agent_1).select { |r| r['jsonmodel_type'] == 'agent_corporate_entity' }.last
    ...
    it 'imports other parts of name' do
      record = convert(corp_agent_2).select { |r| r['jsonmodel_type'] == 'agent_corporate_entity' }.last
