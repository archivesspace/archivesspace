...
class MarcXMLConverter < Converter
  ...
  def self.subject_template(getterms, getsrc)
    {
      :obj => :subject,
      :rel => :subjects,
      :map => {
  ...
  def self.bibliography_note_template(label, template=nil, *tmpl_args)
    {
  ...
  def self.singlepart_note(note_type, label, template=nil, *tmpl_args)
    {
      :obj => :note_singlepart,
      :rel => :notes,
      :map => {
  ...
  def self.multipart_note(note_type, label = nil, template=nil, *tmpl_args)
    {
      :obj => :note_multipart,
      :rel => :notes,
      :map => {
  ...
  def self.adds_agent_term(term_type, prefix = "")
    Proc.new{|agent, node|
      agent['_terms'] ||= []
      make(:term) do |term|
  ...
  def self.sets_other_name_source
    Proc.new {|name, node|
  ...
  def self.sets_use_date_from_code_d
    Proc.new {|name, node|
      make(:date) do |date|
        date.label = 'other'
        date.date_type = 'single'
  ...
  def self.adds_prefixed_qualifier(prefix, separator = ': ')
    Proc.new {|name, node|
      val = node.inner_text
      if val
        name.qualifier ||= ""
  ...
  def self.appends_subordinate_name_2
    Proc.new{|name, node|
  ...
  def self.subfield_template(template, node, map=nil)
  ...
  def self.concatenate_subfields(codearray, node, delim=' ')
    result = ""
  ...
  def self.is_fallback_resource_title
    {
