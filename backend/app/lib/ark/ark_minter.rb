...
class ArkMinter
  ...
  # Caching template lookup for bulk record creation (like imports)
  def template_for_repo(repo_id)
    if RequestContext.get(:repo_ark_templates).nil?
      RequestContext.put(:repo_ark_templates, {})
    end

    templates = RequestContext.get(:repo_ark_templates)

    if !templates.include?(repo_id)
      DB.open do |db|
        templates[repo_id] = db[:repository].filter(:id => repo_id).get(:ark_template)
      end
    end
