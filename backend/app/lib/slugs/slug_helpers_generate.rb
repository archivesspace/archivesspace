module SlugHelpers
  ...
  def self.run_dedupe_slug(slug, suffix = false)
    # search for dupes
    if !slug.empty? && slug_in_use?(slug)
      slug = dedupe_slug(slug, suffix)
    else
      slug
    end
  ...
  def self.dedupe_slug(dupe_slug, suffix)
    suffix ||= Time.now.nsec
    new_slug = "#{dupe_slug}_#{suffix}"

    if slug_in_use?(new_slug)
      dedupe_slug(new_slug, Time.now.nsec)
    else
      return new_slug
    end
  end
