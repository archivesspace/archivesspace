onmessage = async e => postMessage(await fetchWaypoints(e.data));

/**
 * fetchWaypoints
 * @description Fetch one or more waypoints of records
 * @param {object} data - The data the worker receives
 * @param {string} data.resourceUri - The uri of the collection resource,
 * ie: /repositories/18/resources/11861
 * @param {[wpNum, uris]} data.waypointTuples - Array of tuples of waypoint metadata
 ...
 */
async function fetchWaypoints(data) {
  const { waypointTuples, resourceUri } = data;
